...
class TD3():
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.critic_1 = Generator(state_dim, action_dim).to(device)
        self.critic_1_target = Generator_target(state_dim, action_dim).to(device)
        self.discriminator1 = Discriminator().to(device)
        self.critic_2 = Generator(state_dim, action_dim).to(device)
        self.critic_2_target = Generator_target(state_dim, action_dim).to(device)
        self.discriminator2 = Discriminator().to(device)
        ...
        self.discriminator1_optimizer = optim.Adam(self.discriminator1.parameters(), lr=args.learning_rate)
        self.critic_2_optimizer = optim.Adam(self.critic_2.parameters(), lr=args.learning_rate)
        self.discriminator2_optimizer = optim.Adam(self.discriminator2.parameters(), lr=args.learning_rate)
    ...
    def update(self, num_iteration):
        ...
        for i in range(1):
            ...
            noise1 = torch.ones_like(reward).data.normal_(0, 1).to(device)
            # noise1 = noise1.clamp(-args.noise_clip, args.noise_clip)
            current_Q1 = self.critic_1(state, action, noise1)
            target_Q1 = self.critic_1_target(next_state, next_action, noise1)
            target_Q2 = self.critic_2_target(next_state, next_action, noise1)
            target_Q = torch.min(target_Q1, target_Q2)
            target_Q = reward + ((1 - done) * args.gamma * target_Q).detach()
            epsilon = target_Q - current_Q1

            loss_discriminator_Q1 = -torch.mean(torch.log(self.discriminator1(current_Q1.detach())) + \
                                               torch.log(1 - self.discriminator1(target_Q.detach())))

            self.discriminator1_optimizer.zero_grad()
            loss_discriminator_Q1.backward()
            self.discriminator1_optimizer.step()

            loss_generator_Q1 = -torch.mean(torch.log(1 - self.discriminator1(target_Q))) +  (1 / 2) * torch.mean(epsilon ** 2)
            # +  (1 / 2) * torch.mean(epsilon ** 2)

            # + (1/2)*torch.mean(epsilon**2)
            # print(loss_generator_Q1)
            self.critic_1_optimizer.zero_grad()
            loss_generator_Q1.backward()
            self.critic_1_optimizer.step()

            # # Compute target Q-value:
            noise2 = torch.ones_like(reward).data.normal_(0, 1).to(device)
            # noise1 = noise1.clamp(-args.noise_clip, args.noise_clip)
            current_Q2 = self.critic_2(state, action, noise2)
            epsilon2 = target_Q - current_Q2

            loss_discriminator_Q2 = -torch.mean(torch.log(self.discriminator2(current_Q2.detach())) + \
                                                torch.log(1 - self.discriminator2(target_Q.detach())))

            self.discriminator2_optimizer.zero_grad()
            loss_discriminator_Q2.backward()
            self.discriminator2_optimizer.step()

            loss_generator_Q2 = -torch.mean(torch.log(1 - self.discriminator2(target_Q))) + (1 / 2) * torch.mean(
                epsilon2 ** 2)
            # +  (1 / 2) * torch.mean(epsilon ** 2)
            ...
            if num_iteration % 5 == 0:
                ...
                Q = self.critic_1(state, actions, noise1)
