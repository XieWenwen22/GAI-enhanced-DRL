...
class Diffusion(nn.Module):
    def __init__(self, state_dim, action_dim, model, max_action,
                 ...
                 loss_type='l1', clip_denoised=False, explore_solution=False):
        ...
        self.register_buffer('posterior_mean_coef2',
                             (1. - alphas_cumprod_prev) * np.sqrt(alphas) / (1. - alphas_cumprod)) # Eq.10 Left
    ...
    def q_posterior(self, x_start, x_t, t):
        #调用的时候x_start指的是x_0, x_t指的是采样数据, t指的是步数
        posterior_mean = (
                extract(self.posterior_mean_coef1, t, x_t.shape) * x_start +
                extract(self.posterior_mean_coef2, t, x_t.shape) * x_t
        ) #Eq.10 All
        # print("Test:",x_t.shape,t.shape)
    ...
    def p_mean_variance(self, x, t, s):
        # print("X,T,S:",x,t,s)
        # print(x.shape,s.shape,t.shape,self.action_dim,self.state_dim)
        x_recon = self.predict_start_from_noise(x, t=t, noise=self.model(x, t, s)) # self.model = actor_net (MLP)
    ...
    def p_sample(self, x, t, s):
        ...
        # print((model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise).shape)
    ...
    def sample(self, state, *args, **kwargs):
        ...
        # print("---State---", state.shape, batch_size, shape)
